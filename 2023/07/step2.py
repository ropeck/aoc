...
def hand_score(hc):
  ...
  if 'J' in hc:
    ...
    if len(hc) == 0:
      hc['A'] = joker
      joker = 0 
  hc_items = [[n[0], n[1]] for n in hc.items()]
  h = sorted(hc_items, key=lambda n: (n[1], CARD_ORDER.index(n[0])), reverse=True)
  pass
  h[0][1] += joker
  if len(h) == 1:
    return _FIVE_OF
...
class Hand:
  ...
  def sort_str(self):
    str = self.card_str
    for f, t in ('Ta', 'J1', 'Qc', 'Kd', 'Ae'):  # joker
      str = re.sub(f, t, str)
    return str
...
def main(test):
  ...
  hands = sorted(hands, key=lambda n: (hand_score(n.hand), n.sort_str()))

  total = 0
  for rank, h in enumerate(hands, 1):
    print (rank, h, h.sort_str(), hand_score(h.hand))
    total += rank * h.bid
